...
def discover_drones_from_bag(bag_path: Path) -> list:
    """Scan bag topic index for self_localization/pose and return sorted drone namespaces."""
    try:
        import rosbag2_py
    except ImportError:
        return []
    ...
    for info in reader.get_all_topics_and_types():
        if info.name.endswith('/self_localization/pose'):
            namespace = info.name.strip('/').split('/')[0]
            drones.add(namespace)
...
FIGURES_ROOT = Path('/root/aerostack2_ws/paper_aerospace/figures')
...
def plot_results(
    ...
    if output:
        ...
        out_dir = FIGURES_ROOT / bag_name
        out_dir.mkdir(parents=True, exist_ok=True)
        for ext in ('png', 'eps', 'svg'):
            p = out_dir / f'{bag_name}.{ext}'
...
def main() -> None:
    ...
    parser.add_argument(
        '--drones', nargs='+',
        default=None,
        help='Drone namespaces to include (default: auto-detected from bag/world/mission)',
    )
    parser.add_argument(
    ...
    if args.static:
        if args.drones:
            drones = args.drones
        elif world_starts is not DRONE_STARTS_DEFAULT:
            drones = sorted(world_starts.keys())
        else:
            mission_drones, _ = load_mission_json(args.mission)
            drones = mission_drones or sorted(DRONE_STARTS_DEFAULT.keys())
    ...
    if args.drones:
        ...
        drones = discover_drones_from_bag(bag_path)
        if drones:
            print(f'[plot] auto-detected drones: {drones}')
        elif world_starts is not DRONE_STARTS_DEFAULT:
            drones = sorted(world_starts.keys())
            print(f'[plot] drones from world file: {drones}')
        else:
            drones = sorted(DRONE_STARTS_DEFAULT.keys())
            print('[plot] falling back to default drone list', file=sys.stderr)
